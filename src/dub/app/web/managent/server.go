...
type WebManUseCenterServer struct {
	wucCfg  define.WebManUseCenterServerConfig //用户的微服务配置
	logCfg  define.LogConfig                   //日志配置
	regConn common.IConnector                  //与注册服务器连接
	log     *utils.Logger                      //日志对象
}

func (w *WebManUseCenterServer) Init(cfgPath string) {
	//读取配置
	var err error
	w.wucCfg, w.logCfg, err = config.GetWebManUseCenterServerConfig(cfgPath)
	if err != nil {
		fmt.Println(err)
		os.Exit(2)
	}
...
func (w *WebManUseCenterServer) Reg() {
	serverInfo := &define.ModelRegReqServerType{
		Addr:       w.wucCfg.Addr,
		ServerName: define.ServerNameWeb_ManAgentServer,
		ServerType: 2,
...
//注册服务回调函数
func (w *WebManUseCenterServer) RegServiceCallBack(mainId, subId uint16, data []byte) bool {
...
var webUseCenter *WebManUseCenterServer

func NewWebManUseCenterServer() *WebManUseCenterServer {
	if webUseCenter == nil {
		webUseCenter = new(WebManUseCenterServer)
	}
	return webUseCenter
}
