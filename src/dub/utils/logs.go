...
func NewLogger() *Logger {
	if g_LogInstance == nil {
		g_LogInstance = &Logger{out: nil, flag: LstdFlags, level: LEVEL_DEBUG}
	}
...
func (self *Logger) SetConfig(cfg define.LogConfig) {
	self.maxSize = cfg.MaxSize
	self.name = "./log/" + cfg.DeviceName + ".log"
	switch cfg.Level {
	case "INFO":
		self.level = 1
	case "WARN":
	...
	if IsFileExist(self.name) {
		f, err := os.OpenFile(self.name, os.O_RDWR|os.O_APPEND, os.ModeAppend)
		if err == nil {
		...
		f, err := os.Create(self.name) //创建文件
		if err == nil {
			self.out = f
		} else {
			self.out = nil
...
func (self *Logger) Output(calldepth int, prefix string, s string) error {
	...
	self.size = self.size + len(self.buf)
	...
	if self.size >= self.maxSize {
		...
		newpath := self.name + "." + time.Now().Format("20060102-150405")
		err = os.Rename(self.name, newpath)
		if err != nil {
			fmt.Println(err)
		}
